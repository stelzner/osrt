from osrt.utils.nerf import get_extrinsic, transform_points
...
class MultishapenetDataset(IterableDataset):
    ...
    def prep_item(self, data):
        ...
        if self.osrt:
            # Indexes range from 0 to 34, with 1 indicating background. 
            # 0 pixels show up in only a very small number of scenes, and perhaps result from rendering
            # errors. We therefore treat both identically as background.
            ...
            target_masks = np.reshape(masks[target_views], (-1, 34))
            if not self.full_scale:
                target_masks = target_masks[sampled_idxs]
            result['target_masks'] = target_masks         # [p, 34]
            result['input_masks'] = input_masks
